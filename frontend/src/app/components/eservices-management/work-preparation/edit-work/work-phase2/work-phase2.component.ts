...
@Component({
  selector: 'app-work-phase2',
  templateUrl: './work-phase2.component.html',
  styleUrls: ['./work-phase2.component.css'], 
  providers: [EmailsService, NotificationsService]
})


export class WorkPhase2Component implements OnInit {
...
//console.log(this.serviceLine,'MangaementeID: ',this.managementId, 'authuserID: ', this.AuthuserId)
      this.jpactivityService.getWorkpreparationByManagementIdandSl(this.serviceLine, this.managementId, this.AuthuserId).subscribe
      (res => { 
        //.log(res);
        if (res[0]){
  ...
  appNotification(form){
    ...
    this.notificationData = ({
      ...
      'description':'editado', 
      'read':0, 
      'action':  "eservices-management/workpreparation/2/"+ this.managementId + '/' + this.serviceLine ,
      'managementId': this.managementId
      // 'modified':string,
    });
  ...
  emailNotification(){
    ...
    this.maildata = ({
      ...
      'description': "eservices-management/workpreparation/2/"+ this.managementId + '/' + this.serviceLine ,
  }); 
  ...
  saveBombLines(bombLines:NgForm){
     
    if(this.fiEdit == '0'){ //create

      this.jpactivityService.getWorkpreparationByManagementIdandSl(this.serviceLine, this.managementId, this.AuthuserId).subscribe
      (res => { 
        //console.log(res);
        this.workEditedId = res[0]._id;
        // console.log('workEditedId',this.workEditedId)
        bombLines.value.workPreparationId = this.workEditedId;
        bombLines.value.userId = this.AuthuserId;
        // console.log(bombLines.value);
        this.jpactivityService.createFiRequirements(bombLines.value).subscribe
        (res=>{
          //console.log(res);
          M.toast({html: 'Guardado!'})    
        })
       ...
       this.jpactivityService.editFiRequirements(bombLines.value, this.editedFiId ).subscribe
       (res=>{
        //console.log(res);
